...
async fn main() {
    ...
    // println!("Anvil node started at {}", anvil.endpoint_url());
    ...
    let mut rom = File::open("../c8games/BRIX").expect("Unable to open file");
            ...
            match evt {
                ...
                Event::KeyDown {
                    ...
                    if let Some(k) = key2btn(key) {
                        chip8.keypress(U256::from(k), true).call().await.unwrap();
                    }
                }
                Event::KeyUp {
                    keycode: Some(key), ..
                    ...
                    if let Some(k) = key2btn(key) {
                        chip8.keypress(U256::from(k), false).call().await.unwrap();
                    }
                }
                _ => (),
            }
